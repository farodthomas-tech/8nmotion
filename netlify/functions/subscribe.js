exports.handler = async function (event) {
  ...
  try {
    ...
    if (response.status === 200 || response.status === 201) return { statusCode: 200, body: JSON.stringify({ success: true, message: "Subscribed successfully!" }) };
    if (data.title === "Member Exists") return { statusCode: 200, body: JSON.stringify({ success: true, message: "You're already subscribed!" }) };
